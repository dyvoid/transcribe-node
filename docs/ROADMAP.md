...
|---------|-------------|-----|
| Browser-based transcription (drag/drop upload + download) | Transcribe zone between Engine and Live log; upload a file via drag/drop or file picker, view/copy/download the result | [0005](adr/0005-ui-panel-lifecycle-and-transcribe-zone.md) |
| Panel active/inactive lifecycle | Settings panel collapses once the engine is loaded; Transcribe panel stays collapsed until it is | [0005](adr/0005-ui-panel-lifecycle-and-transcribe-zone.md) |
| Model download milestones in console | Log "downloading"/"ready" milestones to the server console during model load | [0006](adr/0006-download-progress-console-logging.md) |
| Anti-repetition defaults | `vad_filter=true` and `condition_on_previous_text=false` by default, overridable per request and in the Transcribe form | — |
